    ...
    <body>
	    ...
	    <g:form action="buscar" method="post" class="medicorequisitante">
			<div class="list">
				...
				<input type="text" placeholder="Buscar: Nome Médico Responsável" id="nome" name="nome" ><br/>
			</div>
                ...
                <div class="card-content yellow lighten-1">
                    ...
                    <g:each in="${medicoresponsavelList}" status="i" var="medicoresponsavel">
                        <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
							...
							<td><g:link action="show" id="${medicoresponsavel.id}">${fieldValue(bean:medicoresponsavel, field:'nome')}</g:link></td>
                        
                            <td><g:link action="show" id="${medicoresponsavel.id}">${fieldValue(bean:medicoresponsavel, field:'crm')}</g:link></td>
                        
                            <td><g:link action="show" id="${medicoresponsavel.id}">${String.format('%td/%<tm/%<tY',medicoresponsavel?.dtCadastro)}</g:link></td>
