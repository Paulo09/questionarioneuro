...
<html>
    <head>
     ...
     <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
	  ...
	  <!--Materilize embarcado-->
        <title>Lista Questionario</title>
    </head>
    ...
    <body style="background-image:url(${createLinkTo(dir:'images/view/list/backgroundPagina',file:'backgroundPagina.jpg')});background-repeat:no-repeat;background-size:cover;">
        <div class="card">
		  ...
		  <span class="card-title"><b>L</b>ista <b>Questionario</b></span>
		</div>
        <div class="body">
            ...
            <div class="list">
                <table class="striped centered grey lighten-2">
                    ...
                    <tbody>
                    <g:each in="${questionarioList}" status="i" var="questionario">
                        <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
                        
                            <td><g:link action="show" id="${questionario.id}">${fieldValue(bean:questionario, field:'id')}</g:link></td>
                        
                            <td>${fieldValue(bean:questionario, field:'descricao')}</td>
                        
                            <td>${fieldValue(bean:questionario, field:'exame')}</td>
                        
                            <td>${fieldValue(bean:questionario, field:'medicorequisitante')}</td>
                        
                            <td>${fieldValue(bean:questionario, field:'medicoresponsavel')}</td>
                        
                            <td>${fieldValue(bean:questionario, field:'nome')}</td>
			...
			<nav class="nav-extended btn waves-effect waves-light" style="background-image:url(${createLinkTo(dir:'images/view/list/backgroundPagina',file:'backgroundPagina.jpg')});">  
				 <ul class="pagination" align="center"><li class="active" style="background-image:url(${createLinkTo(dir:'images/view/list/paginacao',file:'paginacao.jpg')});"><g:paginate total="${Questionario.count()}"/></ul>
            </nav>
        </div>
