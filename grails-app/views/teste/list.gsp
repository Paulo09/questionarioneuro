...
<html>
    ...
    <body style="background-image:url(${createLinkTo(dir:'images/view/list/backgroundPagina',file:'backgroundPagina.jpg')});background-repeat:no-repeat;background-size:cover;">
        <div class="card">
		  ...
		  <span class="card-title"><b>L</b>ista <b>Teste</b></span>
		</div>
        <div class="body">
                ...
                <table class="striped centered grey lighten-2">
                    <thead>
                        <tr>
                   	        ...
                   	        <g:sortableColumn property="nivelEscolaridade" title="Nivel Escolaridade" />
                    ...
                    <tbody>
                    <g:each in="${testeList}" status="i" var="teste">
                        <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
                        
                            <td><g:link action="show" id="${teste.id}">${fieldValue(bean:teste, field:'id')}</g:link></td>
                        
                            <td>${fieldValue(bean:teste, field:'sexo')}</td>
                        
                            <td>${fieldValue(bean:teste, field:'nivelEscolaridade')}</td>
                        
                            <td>${fieldValue(bean:teste, field:'cpf')}</td>
                        
                            <td>${fieldValue(bean:teste, field:'dtCadastro')}</td>
                        
                            <td>${fieldValue(bean:teste, field:'email')}</td>
			...
			<nav class="nav-extended btn waves-effect waves-light" style="background-image:url(${createLinkTo(dir:'images/view/list/backgroundPagina',file:'backgroundPagina.jpg')});">  
				 <ul class="pagination" align="center"><li class="active" style="background-image:url(${createLinkTo(dir:'images/view/list/paginacao',file:'paginacao.jpg')});"><g:paginate total="${Teste.count()}"/></ul>
            </nav>
        </div>
