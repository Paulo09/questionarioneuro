...
class Questionariovideoeeg2Controller {
    ...
    def editar = {
		...
		if(c.save()){render "Questionario video EEG Id:${c.id} - Editado com sucesso!!"
    ...
    def salvar = {
		...
		if(questionariovideoeeg2.save()){
			render "Questionario video EEG Id:${questionariovideoeeg2.id} - Salvo com sucesso!"
    ...
    def delete = {
        ...
        if(questionariovideoeeg2) {
            ...
            flash.message = "Questionario video EEG ${params.id} Apagado"
            redirect(action:list)
        }
        else {
            flash.message = "Questionario video EEG não encontrado id ${params.id}"
    ...
    def create = {
        def questionariovideoeeg2 = new Questionariovideoeeg2([paciente:Paciente.get(params.id)])
        questionariovideoeeg2.properties = params
        return ['questionariovideoeeg2':questionariovideoeeg2]
    }
    ...
    def save = {
        ...
        if(!questionariovideoeeg2.hasErrors() && questionariovideoeeg2.save()) {
            flash.message = "Questionario video EEG ${questionariovideoeeg2.id} cadastrado"
            redirect(action:show,id:questionariovideoeeg2.id)
        }
        else {
